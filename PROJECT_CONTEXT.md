...
- Implemented in this pass: Distill now disables browser Service Worker registration in the Tauri desktop runtime, unregisters any previously registered desktop Service Workers when the frontend can load, and performs a one-time native cleanup of the Windows desktop WebView `LOCALAPPDATA\app.distill.local\EBWebView` cache before startup so stale cached bundles cannot keep the installed app on a white screen after updates.
...
- Primary docs: `docs/project_context.md`, `docs/roadmap.md`, `docs/sync_design.md`, `docs/storage_boundary.md`, `docs/desktop_troubleshooting.md`, `docs/release_notes_0.1.46.md`.
## 役割

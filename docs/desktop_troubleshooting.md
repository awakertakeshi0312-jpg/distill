...
Distill 0.1.47 and later include an automatic desktop fix for the most common cause: stale WebView Service Worker/cache state after an update. The installer clears only the local WebView `LOCALAPPDATA\app.distill.local\EBWebView` cache before app startup and does not touch the encrypted vault database or backups.

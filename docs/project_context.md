...
- Dedicated sync key material stored in encrypted sync metadata, Inspector create/rotate lifecycle controls, in-app recovery drill, and passphrase-wrapped into new packets for first import/recovery
...
Latest verification after sync key recovery drill update:
...
- `src/syncRecoveryDrill.ts`: dry-run verification that local sync-key decrypt and passphrase-wrapped recovery decrypt both work without leaking plaintext sync-key material

...
- Daily journal capture lane next to the thought capture lane; saved journal entries become normal searchable blocks with `#journal` and `[[Daily Journal]]`
...
- Public mobile web URL in the Mobile access panel: `https://awakertakeshi0312-jpg.github.io/distill/`
...
- A/B sync-folder operation drill that validates source export, partner bootstrap, partner project/block return records, return preview, and dry-run merge without changing the active vault
...
Latest verification after multi-device sync key recovery drill update:

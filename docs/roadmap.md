...
- Sync design has record-level encryption, device registry, deletion tombstones, replay/rollback guard, chained checkpoint validation, sync-folder safety classification, assisted recommended preview, encrypted pre-sync recovery snapshots with restore preview, and deterministic conflict strategy before automatic transport.

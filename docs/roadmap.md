...
Distill 0.1.60 is past the local MVP gate. The main current phase is Trust Layer and Sync hardening plus mobile/PWA access hardening: encrypted local vault is implemented, passphrase lifecycle is in place, restore preview is implemented, the capture area now has side-by-side thought and daily journal lanes with an empty thought input by default, and the Mobile access panel surfaces the verified public GitHub Pages URL for phone use. Record-level encrypted sync packets have project metadata records, manual export/import, desktop sync-folder packet exchange, sync-folder safety scan, monitor-only review queue, outbound auto-export for local changes, recommended preview, packet quarantine, risk acknowledgement before destructive applies, encrypted pre-sync recovery snapshots with in-app restore preview, sync preview rollback drill, A/B sync-folder operation drill, device-loss recovery runbook, encrypted vault record-log shadow persistence, device registry, signed device checkpoints, source-device verification codes with QR display/scanner import, known-device forget/removal, safe semi-automatic inbound preview, mobile/PWA install guidance, update-safer offline shell caching, desktop Service Worker disablement plus NSIS preinstall Windows WebView cache cleanup for white-screen recovery, unlock-form compatibility for legacy/short existing passphrases, phone-width PWA smoke coverage, IndexedDB-backed encrypted browser vault persistence, volatile in-memory vault session passphrase handling, non-exportable WebCrypto vault session key persistence, packet-level non-exportable WebCrypto sync session key support, dedicated encrypted-vault sync key material with visible create/rotate lifecycle controls, single-vault and A/B multi-device recovery drills, and wrapped bootstrap support, unknown-device trust confirmation, device trust revocation, deletion tombstones, replay/rollback protection, chained checkpoint validation, and apply-before-confirm sync previews with decision-review counts.
...
- Inbox capture.
- Side-by-side daily journal capture lane that saves entries as searchable `#journal` / `[[Daily Journal]]` blocks.
...
- Desktop white-screen recovery hardening by disabling browser Service Worker registration in Tauri and clearing stale WebView cache once on startup.
...
- Record-level encrypted records. Complete for project records, manual sync packet export/import with apply preview, deletion tombstones, device registry, device trust revocation, known-device forget/removal, stale packet rejection, chained checkpoint validation, sync decision-review counts, risk acknowledgement, encrypted pre-sync recovery snapshots with restore preview, sync-folder safety scan, monitor-only review queue, outbound auto-export for local changes, recommended preview, packet quarantine, explicit desktop sync-folder packet exchange, and encrypted vault record-log shadow persistence that can replay projects, blocks, tombstones, device records, revoked devices, and sync-key material. Automatic inbound apply/cloud sync and primary record-log normal persistence are not enabled yet.
...
- Project metadata sync records.
- Encrypted vault record-log shadow persistence for future normal persistence and compaction.
...
- Sync preview rollback drill that dry-runs packet apply and confirms the pre-sync snapshot can be restored without changing the active vault.
- A/B sync-folder operation drill that simulates source export, partner bootstrap, partner project/block creation, return preview, and dry-run merge without changing the active vault.
- Device-loss recovery runbook that scores readiness across sync key, verification code, partner device, sync folder, recovery snapshots, dry-run drills, and rollback drill availability.
...
- Sync design has record-level encryption, encrypted vault record-log shadow persistence, device registry, deletion tombstones, replay/rollback guard, chained checkpoint validation, sync-folder safety classification, monitor-only review queue, outbound auto-export, assisted recommended preview, signed device checkpoints, source-device verification codes with QR display/scanner import, known-device forget/removal, unknown-device trust confirmation, encrypted pre-sync recovery snapshots with restore preview, sync preview rollback drill, A/B sync-folder operation drill, device-loss recovery runbook, packet-level sync session KDF metadata, dedicated sync-key bootstrap, lifecycle controls, single-vault and A/B recovery drills, and deterministic conflict strategy before automatic transport.

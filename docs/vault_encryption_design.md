...
Distill 0.1.38 uses an encrypted local vault for normal app persistence.
...
- Manual sync packets use record-level encrypted records, packet-level sync KDF metadata, dedicated encrypted-vault sync key material with create/rotate lifecycle controls, passphrase-wrapped sync key bootstrap metadata, device registry metadata, revoked device metadata, and deletion tombstones.
